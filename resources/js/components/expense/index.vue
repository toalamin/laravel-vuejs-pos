        ...
        <div class="card card-default">
            ...
            <div class="card-body">
                <table class="table table-bordered table-striped text-sm table-vcenter table-condensed">
                    ...
                    <tbody>
                        <tr v-for="(expense,index) in expenseList.data" :key="expense.id">
                            <td>{{index+1}}</td>
                            <td>{{ expense.expense_date }}</td>
                            <td>{{ expense.details }}</td>
                            <td>{{ expense.amount }}</td>
                            <td nowrap>
                          <router-link :to="{name: 'edit-expense', params:{id: expense.id} }" class="btn btn-sm btn-info"><i class="fas fa-edit"></i></router-link>
                        <a @click="deleteItem(expense.id)" class="btn btn-sm btn-danger"><i class="fas fa-trash"></i></a>
                            </td>
                        </tr>
                    </tbody>
    ...
    methods: {
        ...
        searchGivenInput(page) {
            if(this.searchTerm.length > 0){
                ...
                let route = '/api/expense?&search_input='+ this.searchTerm +'&page=' + page;
                axios.get(route)
                    .then((response) => {
                        this.expenseList = response.data;
                    });
